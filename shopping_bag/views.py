    ...
    if size:
        if item_id in list(current_bag.keys()):
            if size in current_bag[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'You have just updated {size.upper()}{product.name} \
                    quantity to {current_bag[item_id]["items_by_size"][size]}')
            else:
                # adding new product with size to cart
                current_bag[item_id]['items_by_size'][size] = item_quantity
                messages.success(request, f'You have just added {size.upper()}{product.name} \
                    to your cart')
        else:
        ...
        if item_id in list(current_bag.keys()):
            ...
            messages.success(request, f'Quantity for {product.name} \
                has been updated to {current_bag[item_id]}')
        else:
            # adding product quantity to cart
            current_bag[item_id] = item_quantity
            messages.success(request, f'Thankyou for adding \
...
def update_bag(request, item_id):
    ...
    if size:
        if item_quantity > 0:
            ...
            current_bag[item_id]['items_by_size'][size] = item_quantity
            messages.info(request, f'You have just updated the quantity of \
                {size.upper()}{product.name} \
                    to {current_bag[item_id]["items_by_size"][size]}')
        else:
            # Deleting a product
            del current_bag[item_id]['items_by_size'][size]
            if not current_bag[item_id]['items_by_size']:
                current_bag.pop(item_id)
            messages.info(request, f'You have just removed size: \
                {size.upper()} {product.name} from your cart')
    else:
        if item_quantity > 0:
            # updating quantity for a product
            current_bag[item_id] = item_quantity
            messages.info(request, f'Quantity for {product.name} \
                has been updated to {current_bag[item_id]}')
        else:
            # Deleting product
            current_bag.pop(item_id)
            messages.info(request, f'You have just removed \
...
def delete_item(request, item_id):
    ...
    try:
        ...
        messages.error(request, f'Ooops it looks like there \
            was an error removing: {e}')
